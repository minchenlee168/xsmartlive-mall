...
const countryOptions = computed(() =>
  countries.map((c) => ({ label: c.label, value: c.key })),
);
const phoneCodeOptions = computed(() =>
  countries.map((c) => ({ label: c.phoneCode, value: c.phoneCode })),
);
...
<template>
  ...
  >
    <template #header>
      <span class="font-bold text-slate-950" style="font-size: 16px"
        >更換配送地址</span
      >
    </template>
    ...
    <div class="flex flex-col gap-4 pt-2">
      ...
      <div class="flex flex-col gap-1.5">
        <label class="text-sm font-medium text-slate-700">配送國別</label>
        <Select
          v-model="countryKey"
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <!-- 多欄式（中、日、美） -->
        <div v-if="country.addressFields" class="grid grid-cols-2 gap-3">
          <div
